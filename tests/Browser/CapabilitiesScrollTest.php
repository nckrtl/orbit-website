...
it('reveals capabilities from the right, holds them readable, and fades left without a grid-wide filter', function (int $width, bool $reduced) {
    ...
    $page->assertScript('async () => {
        ...
        if(states.some(s=>s.filter!=="none")) return false;
        if(reduced) return states.every(s=>s.opacity===1 && s.x===0 && !s.inert);
        return before.opacity===0 && before.x>0 && before.inert
            && entering.opacity>.4 && entering.opacity<.6 && entering.x>0 && entering.x<before.x
            && [entered,middle,held,back].every(s=>s.opacity===1 && s.x===0 && !s.inert)
            && exiting.opacity>.4 && exiting.opacity<.6 && exiting.x<0
            && after.opacity===0 && after.x<exiting.x && after.inert;
